...
for (d in 2:p){
  ...
  for (l in 1:runs){
    ...
    # save log
    log.file <- sprintf("results/pmc_hd/log_%s_%d_%d_%d_%s_%s_%s.txt",
  ...
  file <- sprintf("results/pmc_hd/%s_p%d_%d_%d_%d_%s_%s_%s.csv", 
                  logf.label, d, K, J, steps, sampling, resampling, sigma.label) 
