---
title: "Improving PMC Using Support Points"
...
Consider resmaple n = 25 points from 2000 Sobol points over the unit square as importance samples for the mixture of normals using Multinomial, Residual, Stratified, Systematic, and Support Points.
...
Let us now run PMC and PQMC on the mixture of five normals with N = 25, J = 10, and T = 6. The initial centers are the 25 Sobol points over the unit square. Adaptation for covariance is applied. Here follows the parameter setting.

```{r pmc-setting, warning = F, message = F}
...
```{r pmc-mn, warning = F, message = F, fig.height = 6, fig.width = 9}
layout(matrix(c(1:6), nrow = 2, byrow = T))
...
```{r pmc-ss, warning = F, message = F, fig.height = 6, fig.width = 9}
layout(matrix(c(1:6), nrow = 2, byrow = T))
...
### PQMC (QMC + SP)
```{r pmc-sp1, warning = F, message = F, fig.height = 6, fig.width = 9}
layout(matrix(c(1:6), nrow = 2, byrow = T))
...
### PQMC (SP + SP)
```{r pmc-sp2, warning = F, message = F, fig.height = 6, fig.width = 9}
layout(matrix(c(1:6), nrow = 2, byrow = T))
pmc.sp <- pmc(ini, logmixture, J, steps, sigma, 
...
### PQMC (QMC + SP)
```{r pmc-banana-sp1, warning = F, message = F, fig.height = 6, fig.width = 9}
layout(matrix(c(1:6), nrow = 2, byrow = T))
...
### PQMC (SP + SP)
```{r pmc-banana-sp2, warning = F, message = F, fig.height = 6, fig.width = 9}
layout(matrix(c(1:6), nrow = 2, byrow = T))
pmc.sp <- pmc(ini, logbanana, J, steps, sigma, 
              sample = "sp", resample = "sp",
              sigma.adapt = adaptation, visualization = T)
